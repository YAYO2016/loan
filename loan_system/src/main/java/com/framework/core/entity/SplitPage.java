...
public class SplitPage {
	...
	public String toSplitPageSQL(String sql) {
		...
		if("MYSQL".equals(GlobalConst.DIALECT)){
		...
		}else if("ORACLE".equals(GlobalConst.DIALECT)){
			buf.append("SELECT * FROM (SELECT T.*, ROWNUM RN FROM (");
			if (!(this.orderBy.isEmpty())) {
				buf.append("select * from ("+sql+") ");
				buf.append(" ORDER BY ");
				Iterator iter = this.orderBy.keySet().iterator();
				Object key = null;
				while (iter.hasNext()) {
			...
			buf.append(") T WHERE ROWNUM <= ");
			buf.append(this.end);
			buf.append(") WHERE RN >= ");
			buf.append(this.start + 1);
		}
		return buf.toString();
	}
